...
print("🔌 Conectando a Supabase para iniciar la importación inteligente...")

# 1. Comprobar / Crear Proveedor "Argomanza S.L."
NOMBRE_PROV = "Argomanza S.L."
res_prov = client.table("proveedores").select("id").eq("nombre_empresa", NOMBRE_PROV).execute()
if res_prov.data:
    prov_id = res_prov.data[0]['id']
    print(f"🏢 Proveedor '{NOMBRE_PROV}' localizado (ID: {prov_id}).")
else:
    print(f"🏢 Proveedor '{NOMBRE_PROV}' no existe. Creando ficha...")
    res_ins_p = client.table("proveedores").insert({"nombre_empresa": NOMBRE_PROV}).execute()
    prov_id = res_ins_p.data[0]['id']

# 2. Leer base de datos para filtro anti-duplicados y cálculo de SKUs
...
# 3. Función generadora de SKUs ("OW-XXX")
contador_sku = 1
def generar_sku():
    global contador_sku
    while True:
...
# 4. Datos Crudos (Tal cual me los pasaste)
datos_raw = """
...
# 5. Procesamiento y Lector
lineas = datos_raw.strip().split('\n')
insertados = 0
omitidos = 0

print("🔄 Procesando líneas del catálogo...")
for linea in lineas:
    ...
    # Buscamos los precios de derecha a izquierda
    precios = []
    indices = []
    for i in range(len(partes)-1, -1, -1):
        # Patrón exacto para atrapar números con comas (Ej: 15,65)
    ...
    if len(precios) == 2:
        pvp_str = precios[0] # El último encontrado
        coste_str = precios[1] # El penúltimo encontrado
        idx_fin_nombre = indices[1]
        ...
        if nombre.lower() in nombres_existentes:
            print(f"  ⚠️ Omitido: '{nombre}' (Ya existía en la base de datos).")
            omitidos += 1
            continue
            
        nuevo_sku = generar_sku()
        
        # 6. Inyección a la Base de Datos
        try:
            res_ins = client.table("productos").insert({
                ...
                "categoria": "Producto",
                "precio_base": coste,
                "igic_tipo": 3.0, # Aplicamos el 3% pactado
                "precio_pvp": pvp,
                "stock_actual": 0,
                "stock_minimo": 2,
                "cantidad_reponer": 5
            ...
            if res_ins.data:
                ...
                # Vinculamos al proveedor Argomanza S.L.
                ...
                insertados += 1
                print(f"  ✅ Insertado: [{nuevo_sku}] {nombre} (PVP: {pvp}€ | Coste: {coste}€ | IGIC: 3%)")
                
        except Exception as e:
            print(f"  ❌ Error al insertar '{nombre}': {e}")
            omitidos += 1

print(f"\n🎉 ¡Magia completada! Se insertaron y vincularon {insertados} productos correctamente. {omitidos} fueron omitidos.")