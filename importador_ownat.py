...
# 1. Asegurar que van directamente al proveedor Argomanza
...
# --- AUTO-LIMPIEZA DE RASTROS DE OWNAT ---
print("🧹 Haciendo limpieza previa profunda de Ownat...")
res_basura = client.table("productos").select("id").ilike("nombre", "OWNAT%").execute()
borrados = 0
if res_basura.data:
    for p in res_basura.data:
...
# 2. Nombres existentes
res_prod = client.table("productos").select("sku, nombre").execute()
nombres_existentes = {str(p.get('nombre', '')).strip().lower() for p in res_prod.data} if res_prod.data else set()

# --- GENERADOR BLINDADO DE CÓDIGOS ---
res_ow = client.table("productos").select("sku").like("sku", "OW-%").execute()
max_num = 0
if res_ow.data:
    for item in res_ow.data:
        try:
            num = int(item['sku'].split('-')[1])
            if num > max_num: max_num = num
        except: pass

contador_sku = max_num + 1
print(f"🔢 El primer código será: OW-{contador_sku:03d}")

def generar_sku():
    global contador_sku
    ns = f"OW-{contador_sku:03d}"
    contador_sku += 1
    return ns
...
for linea in lineas:
    ...
    if len(precios) == 2:
        pvp_str = precios[0]
        coste_str = precios[1]
        idx_fin_nombre = indices_precios[1]
...
print(f"\n🎉 ¡Magia completada! {insertados} productos de Ownat insertados a nombre de Argomanza. {omitidos} omitidos.")
