...
def render_pestana_tareas(client):
    ...
    if tab_gerencia:
        with tab_gerencia:
            opciones_jefes = ["Jefatura (Ambos)", "Jefe 1", "Jefe 2"]
            render_seccion_tareas(client, "👔 3. Funciones de Gerencia", "💡 Ej: 'Pagar nóminas', 'Revisión fiscal', 'Entrevista personal nuevo', etc.", "Gerencia", opciones_jefes)
            
    if tab_proyectos:
        with tab_proyectos:
            st.markdown("#### 🚀 4. Módulo de Proyectos a Largo Plazo")
            st.info("💡 Espacio exclusivo para gerencia: planifica ideas, reformas, lanzamiento de nuevos servicios o eventos.")
            
            c_p1, c_p2 = st.columns([1, 2.5])
            
            with c_p1:
                with st.form("nuevo_proyecto", clear_on_submit=True):
                    st.markdown("##### ➕ Nueva Idea / Proyecto")
                    p_nom = st.text_input("Nombre del Proyecto *", key=f"p_nom_{st.session_state.llave_proyecto}")
                    p_desc = st.text_area("Descripción / Notas", key=f"p_desc_{st.session_state.llave_proyecto}")
                    p_prio = st.selectbox("Prioridad", ["Alta 🔥", "Media ⚡", "Baja ⏳"], index=1, key=f"p_pri_{st.session_state.llave_proyecto}")
                    
                    if st.form_submit_button("Crear Proyecto", type="primary", use_container_width=True):
                        if p_nom:
                            try:
                                client.table("proyectos_internos").insert({
                                    "proyecto": p_nom,
                                    "descripcion": p_desc,
                                    "prioridad": p_prio,
                                    "estado": "Idea 💡"
                                }).execute()
                                st.session_state.llave_proyecto += 1
                                st.success("Proyecto creado."); time.sleep(0.5); st.rerun()
                            except Exception as e:
                                st.error("⚠️ Falta crear la tabla 'proyectos_internos' en Supabase.")
                        else:
                            st.warning("El nombre del proyecto es obligatorio.")
                            
            with c_p2:
                try:
                    res_pro = client.table("proyectos_internos").select("*").order("created_at", desc=True).execute()
                    if res_pro.data:
                        df_p = pd.DataFrame(res_pro.data)
                        df_vista_p = df_p[['id', 'proyecto', 'descripcion', 'prioridad', 'estado']].copy()
                        df_vista_p.insert(0, "Borrar", False)
                        
                        ed_p = st.data_editor(
                            df_vista_p, hide_index=True, use_container_width=True, height=400,
                            column_config={
                                ...
                                "estado": st.column_config.SelectboxColumn("Estado", options=["Idea 💡", "Planificado 📅", "En progreso 🏗️", "Completado ✅", "Descartado ❌"]),
                                "id": None
                            }
                        )
                        ...
                        if st.button("💾 Guardar Cambios en Proyectos", type="primary"):
                            ...
                            for _, rb in filas_borrar.iterrows():
                                client.table("proyectos_internos").delete().eq("id", rb['id']).execute()
                                
                            filas_validas = ed_p[ed_p["Borrar"] == False]
                            for _, rv in filas_validas.iterrows():
                                client.table("proyectos_internos").update({
                                    "proyecto": str(rv['proyecto']),
                                    "descripcion": str(rv['descripcion']),
                                    "prioridad": str(rv['prioridad']),
                                    "estado": str(rv['estado'])
                                }).eq("id", rv['id']).execute()
                            st.success("Proyectos actualizados."); time.sleep(0.5); st.rerun()
                    else:
                        st.info("Aún no hay proyectos registrados.")
                except:
                    st.info("🔧 Los proyectos aparecerán aquí cuando crees la tabla 'proyectos_internos'. Columnas necesarias: proyecto, descripcion, prioridad, estado.")