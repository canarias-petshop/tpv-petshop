...
def fetch_citas_roi(_client, fecha_ini, fecha_fin):
    res = _client.table("citas").select("fecha_hora, servicio, mascota_id").gte("fecha_hora", fecha_ini).lt("fecha_hora", fecha_fin).execute()
    citas = res.data or []
...
def render_pestana_estadisticas(client):
    with st.container():
        ...
        if res_ventas_prev.data:
            ...
            if not df_vp.empty and 'total' in df_vp.columns:
                df_vp['total'] = pd.to_numeric(df_vp['total'], errors='coerce').fillna(0.0)
                total_ventas_prev = df_vp[df_vp['estado'] != 'DEVUELTO']['total'].sum()
