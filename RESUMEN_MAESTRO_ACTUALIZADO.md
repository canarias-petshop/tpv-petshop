...
- **Ecosistema de Scripts Auxiliares y Parsing Inteligente (NUEVO):** 
  - **Importadores Inteligentes de PDF:** Creación de un ejército de scripts independientes (`importador_zotecnica.py`, `importador_ownat.py`, `importador_gloria_pets.py`, `importador_julius.py`, `importador_cunipic.py`, `importador_earth_rated.py`, `importador_kong.py`, `importador_stangest.py`, `importador_beaphar.py`, `importador_boehringer.py`, `importador_elanco.py`, `importador_flexi.py`, `importador_ceva.py`, etc.) equipados con un **"Radar Ultra-Inteligente"** capaz de leer texto bruto, ignorar cabeceras inútiles, extraer códigos de barras EAN, traducir símbolos HTML y asignar dinámicamente prefijos SKU únicos y un **3% de IGIC** automático.
  - **Herramientas de Limpieza y Corrección Masiva:** Scripts para purgar catálogos antiguos protegiendo las nuevas importaciones (`limpiar_productos_antiguos.py`), y utilidades quirúrgicas para cambiar SKUs o actualizar impuestos masivamente en productos ya subidos (`actualizar_sku_amanova.py`, `actualizar_igic_zootecnia.py`, `limpiar_ownat.py`, `arreglar_proveedores_zootecnia.py`).
  - **Rescate de Clientes y Copias de Seguridad:** Scripts para rescate de datos huérfanos (`2_rescate_clientes_tienda.py`, `3_rescate_fichas_rebeldes.py`), extracción de Excel (`importador_excel.py`, `importador_gastos.py`) y un sistema de **Copias de Seguridad Automáticas** blindado (`backup_total_automatico.py` / `descargar_todos_los_datos.bat`) que permite extraer toda la base de datos de la nube a local en un clic.
