...
- **Ecosistema de Scripts Auxiliares (NUEVO):** Creación de scripts independientes para migraciones masivas (`importador_excel.py`, `importador_gastos.py`, `importador_productos.py`, **`importador_zotecnica.py`** para parseo directo de PDFs). Herramientas de limpieza y corrección de catálogos (`1_limpieza_segura.py`, **`limpiar_zotecnica.py`**), rescate de datos huérfanos (`2_rescate_clientes_tienda.py`, `3_rescate_fichas_rebeldes.py`), utilidades menores (**`listar_archivos.py`**) y un sistema de **Copias de Seguridad Automáticas** (`backup_total_automatico.py` / `descargar_base_datos.py` / `descargar_todos_los_datos.bat`) que permite extraer toda la base de datos de la nube a local en un clic.
