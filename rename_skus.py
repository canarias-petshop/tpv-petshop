...
# Obtener todos los productos de AMANOVA
res = supabase.table("productos").select("id, sku, nombre, mascota, familia, subcategoria").eq("marca", "AMANOVA").execute()
productos = res.data

# Función para ordenar los productos
# Orden deseado: Mascota (Perro > Gato) -> Familia -> Nombre (para que las cajas y unidades queden juntas)
def sort_key(p):
...
print(f"Total productos a procesar: {len(productos_ordenados)}")

updated_count = 0
for idx, p in enumerate(productos_ordenados, start=1):
    nuevo_sku = f"AM-{idx:03d}" # AM-001, AM-002, etc.
    viejo_sku = p.get("sku")
    p_id = p.get("id")
...
print(f"\nMigración completada. Productos con nuevo SKU asignado: {updated_count}")
