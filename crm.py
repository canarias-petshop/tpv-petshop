        ...
        with sub_domicilio:
            ...
            with col_d1:
                ...
                with st.form("n_domicilio", clear_on_submit=True):
                    opc_cli_dom = ["👤 Cliente no registrado (Escribir a mano)"]
                    if res_clientes.data:
                        opc_cli_dom += [f"{c['nombre_dueno']} | {c['telefono']}" for c in res_clientes.data]
                    
                    sel_cli_dom = st.selectbox("1. Buscar Cliente:", opc_cli_dom, key=f"nd_sel_{st.session_state.llave_crm_dom}")
                    ...
                    with c_nom_d: d_cli_man = st.text_input("Nombre", key=f"nd_nom_{st.session_state.llave_crm_dom}")
                    with c_tel_d: d_tel_man = st.text_input("Teléfono", key=f"nd_tel_{st.session_state.llave_crm_dom}")
                    d_dir_man = st.text_input("Dirección de entrega", key=f"nd_dir_{st.session_state.llave_crm_dom}")
                    
                    st.markdown("---")
                    d_prod = st.text_area("2. Detalle del pedido *", key=f"nd_prod_{st.session_state.llave_crm_dom}")
                    ...
                    if st.form_submit_button("Guardar Pedido", type="primary", use_container_width=True):
                        ...
                        if "Cliente no registrado" not in sel_cli_dom:
                            if not final_cli: final_cli = sel_cli_dom.split(" | ")[0]
                            if not final_tel: final_tel = sel_cli_dom.split(" | ")[1] if len(sel_cli_dom.split(" | ")) > 1 else ""
                            if not final_dir:
                                try:
                                    cli_info = next(c for c in res_clientes.data if c['nombre_dueno'] == final_cli)
                                    final_dir = cli_info.get('direccion', '')
                                except: pass
                        ...
                        if final_cli and d_prod:
                            try:
                                ...
                                st.session_state.db_version = st.session_state.get('db_version', 0) + 1
                                st.session_state.llave_crm_dom += 1
